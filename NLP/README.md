...
**학습되는 것은 게이트 값을 만드는 W와 b입니다.** 학습된 파라미터에 현재 입력과 이전 상태를 넣어서 매 시점 새로운 게이트 값을 계산합니다. 정답 예측의 손실이 시간축을 따라 역전파되면서 어떤 정보를 유지·기록·노출하는 것이 유리한지 학습됩니다. 이 시간축 역전파를 **BPTT(Backpropagation Through Time)** 라고 하며, 기본 RNN도 같은 방식으로 학습합니다. Output gate는 hidden state를 조절하고, 어휘 출력층은 그 hidden state로 토큰 확률을 계산하므로 역할과 파라미터가 다릅니다.
...
그림의 `c = h₄`는 입력 요약(context)을 hidden state 하나로 축약해 표시한 것입니다. 이 c는 LSTM의 cell state 기호와 다른 의미이고, 위 수식은 cell state까지 포함해 전달 상태를 명시했습니다. `<BOS>`는 생성 시작, `<EOS>`는 생성 종료를 나타내는 특수 토큰이며, 첫 입력 $y_0$는 `<BOS>`입니다.
...
$$
p_i = \mathrm{softmax}(z_i), \qquad P(y_i = v \mid y_{<i}, x) = p_i[v]
$$

여기서 $N_{\mathrm{vocab}}$은 어휘 집합의 크기, v는 후보 토큰의 ID, $y_{<i}$는 이전 출력 토큰들, x는 전체 입력 문장입니다. $p_i[v]$는 확률분포에서 후보 v에 해당하는 값입니다. 가장 확률이 높은 토큰을 고르는 **greedy**나 확률에 따라 뽑는 **sampling**으로 다음 토큰을 정할 수 있습니다.
...
$$
e_{ij} = v_a^\top \tanh(W_s h_{i-1}^{dec} + W_h h_j^{enc} + b_a)
$$
...
$$
a_i = \sum_j \alpha_{ij} h_j^{enc}
$$

$$
s_i = \mathrm{LSTM}_{dec}([E(y_{i-1}), a_i],\, s_{i-1})
$$
