...
|분산 데이터베이스|물리적으로 떨어진 데이터베이스를 네트워크로 연결하여 단일 데이터베이스 이미지를 보여준다. <br>분산된 작업 처리를 수행하는 데이터베이스로 사용자는 시스템이 분산되어 있는지 인식하지 못한 채 사용할 수 있다.|
...
|클라우드 데이터베이스|클라우드 플랫폼을 통해 구축/접근할 수 있는 데이터베이스로 기존 데이터베이스와 동일한 기능에 클라우드 컴퓨팅의 유연성이 추가되었다. <br>사용자가 클라우드 인프라에서 DBMS를 선택하여 데이터베이스를 구현할 수 있다.|
